...
void zdj_hmi_activate( void ) {
    ...
    // Start HMI scan thread
    pthread_t prescan_thread;
    hmi_scan_thread_running = true;
    pthread_create( &prescan_thread, NULL, hmi_input_scan_thread_main, NULL );
}

void zdj_hmi_deactivate( void ) {
    // zdj_m7_msg( ZDJ_M7_DEACTIVATE_HMI );

    // Signal HMI Scan thread to stop
    hmi_scan_thread_running = false;
}
