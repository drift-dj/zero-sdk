...
static void _get_edge_data( void * _deck, zdj_pipeline_node_t * data_pipe, bool stereo ) {
    ...
    if( stereo && tsm_pitch_state->channel_count == 2 ) {
        // stereo->stereo
        memcpy( soundcard_buf, tsm_pitch_state->out_buffer, ZDJ_SOUNDCARD_BUF_LEN * 2 * sizeof( float ) );
    } else if( !stereo && tsm_pitch_state->channel_count == 1 ) {
        // mono->mono
...
static void * _pipeline_thread_main( void * arg ) {
    ...
    while( !deck_state->exit_thread ) {
        ...
        // Feed new needle head to pitch tsm node state
        tsm_pitch_state->decode_end_coord = deck->controls.platter.needle.head;
        tsm_pitch_state->decode_buf_ref_coord = decode_state->head_win_start;
