...
static void _update_wait( zdj_pipeline_node_t * node ) {
    ...
    //     int sample_offset = state->decode_coord - decode_state->head_win_start;
    ...
    //             if( state->decode_coord > decode_state->head_win_end ) { /*printf( "TSM Tried to read beyond end of decode window\n" ); */continue; }
    ...
    //     // There's no notion of where this buffer goes in the larger stream of outout samples
    //     for ( int q=0; q<len; q++ ) {
