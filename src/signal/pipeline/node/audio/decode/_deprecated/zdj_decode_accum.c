...
static bool _make_buffer_indexes( 
    ...
    int node_win_start = state->head_win_start;
    int node_win_end = state->head_win_end;

    // Bug out if this packet is outside the window
    if( packet->core_start_addr > node_win_end ) { return false; } 
    if( packet->core_end_addr < node_win_start ) { return false; } 
    ...
    packet_clipped_end = fmin( 
        ...
        packet->core_end_addr // decode-space address of last sample in packet
    );
    // Sample copy count
    ...
    *out_buffer_start = out_sample_start * state->channel_count;
    ...
    //     packet->core_start_addr,
    //     packet->core_end_addr,
    //     packet->av_frame_sample_count,
    //     packet->core_end_addr - packet->core_start_addr
    // );
    // if( packet->core_start_addr > node_win_start ) {
    //     printf( "%d[ |%ld ", node_win_start, packet->core_start_addr );
    // } else {
    //     printf( "|%ld %d[ ", packet->core_start_addr, node_win_start );
    // }

    // if( packet->core_end_addr < node_win_end ) {
    //     printf( "%ld| ]%d", packet->core_end_addr, node_win_end );
    // } else {
    //     printf( "]%d %ld|", node_win_end, packet->core_end_addr );
    // }

    // printf( "    [ %d | %d | %d ]", packet_clipped_start, *sample_count, packet_clipped_end );

    // printf( "    p %d  o %d\n", packet_sample_start, out_sample_start );

    // printf( "pkt: [ws:%d cs:%ld ce:%ld we:%d] s:%d e%d\n", 
    //     node_win_start,
    //     packet->core_start_addr, 
    //     packet->core_end_addr,
    //     node_win_end,
    //     packet->core_start_addr > node_win_end,
    //     packet->core_end_addr < node_win_start
    // );
...
int zdj_decode_node_accum_flt( zdj_pipeline_node_t * node, zdj_decode_packet_t * packet ) {
    ...
    int packet_buffer_start, out_buffer_start, sample_count;
    if( !_make_buffer_indexes( node, packet, &packet_buffer_start, &out_buffer_start, &sample_count ) ) {
        return 0;
    }

    // printf( "accum: %ld nod_win:%ld|%ld|%ld pkt_s/e:%ld|%d|%ld ps:%d os:%d cnt:%d\n", 
    //     packet->id,
    //     state->head_win_start,
    //     state->win_sample_count,
    //     state->head_win_end,
    //     packet->packet_pcm_addr,
    //     packet->av_frame_sample_count,
    //     packet->packet_pcm_addr + packet->av_frame_sample_count - 1,
    //     packet_buffer_start / state->channel_count,
    //     out_buffer_start / state->channel_count,
    //     sample_count
    // );
