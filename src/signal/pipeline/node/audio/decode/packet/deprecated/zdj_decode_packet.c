...
int zdj_decode_packet( 
    ...
    while ( !exit ) {
        ...
        res = avcodec_send_packet( state->codec_ctx, packet->av_packet );
        if ( res >= 0 ) { 
        ...
        attempt++;
        // printf( "rf\n" );
        av_read_frame( state->fmt_ctx, packet->av_packet );
        // printf( "rf done\n" );
    }
    ...
    res = 0;
    res = avcodec_receive_frame( state->codec_ctx, packet->av_frame );
    ...
    if ( res == AVERROR( EAGAIN ) || res == AVERROR_EOF ) {
        ...
        decoded_frame_count = packet->av_frame->nb_samples;
    }
...
// Decode and discard samples from a backward seek.
void zdj_decode_garbage_packet( zdj_pipeline_node_t * node, zdj_decode_packet_t * packet ) {
    // printf( "zdj_decode_garbage_packet: %p\n", packet );
...
zdj_decode_packet_t * zdj_decode_get_packet_under_head( 
    ...
    // printf( "zdj_decode_get_packet_under_head\n" );
    ...
    // printf( "zdj_decode_get_packet_under_head not found\n" );
    return NULL;
}

void zdj_decode_deinit_packet( zdj_decode_layer_t * layer, zdj_decode_packet_t * packet ) {
    ...
    if( packet == layer->first_packet ){ layer->first_packet = packet->next; }
    if( packet == layer->last_packet ){ layer->last_packet = packet->prev; }
    packet->next = NULL;
    packet->prev = NULL;
    if( packet->av_packet ){ av_packet_unref( packet->av_packet ); av_packet_free( &packet->av_packet ); }
    if( packet->av_frame ){ av_frame_unref( packet->av_frame ); av_frame_free( &packet->av_frame ); }
...
void zdj_decode_make_admin_packet( 
    ...
    zdj_decode_dir_t direction,
    int len 
) {
    if( direction == ZDJ_DECODE_DIR_BACK ) { 
        packet->type = type;
    ...
    } else if( direction == ZDJ_DECODE_DIR_FWD ) { 
        packet->type = type;
        packet->av_frame_sample_count = len;
