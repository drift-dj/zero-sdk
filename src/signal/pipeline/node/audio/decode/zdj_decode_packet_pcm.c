...
// bool zdj_decode_pcm_layer_can_add_packet_before( 
...
//         layer->first_packet->type == ZDJ_DECODE_PACKET_TYPE_DISCON // first_packet has been marked 'discon' during decode
//     ) {
//         return false;
//     } else {
//         return true;
...
// void zdj_decode_pcm_layer_can_add_packet_after( 
//     zdj_pipeline_node_t * node, 
//     zdj_decode_layer_t * layer, 
//     int64_t address 
// ) {
//     if( layer->last_packet->core_end_addr > address || // first_packet extends earlier than the given address
//         layer->last_packet->type == ZDJ_DECODE_PACKET_TYPE_DISCON // first_packet has been marked 'discon' during decode
//     ) {
//         return false;
//     } else {
//         return true;
...
void zdj_decode_add_packet_before_pcm_layer( zdj_pipeline_node_t * node, zdj_decode_layer_t * layer ) {
    ...
    new_packet->av_timebase_factor = 1;
    ...
    if ( linked_packet->type == ZDJ_DECODE_PACKET_TYPE_START_INERT ) {
        ...
        zdj_decode_make_admin_packet( 
            ...
            ZDJ_DECODE_PACKET_TYPE_START_BUMPER, ZDJ_PACKET_DIRECTION_BACK,
            10000
        );
        ...
        zdj_decode_make_admin_packet( 
            layer, new_packet, linked_packet,
            ZDJ_DECODE_PACKET_TYPE_START_INERT, ZDJ_PACKET_DIRECTION_BACK,
            1000
        );
    ...
    if( packet_seek_timestamp == 0 ) {
        new_packet->has_sof = true;
    }
    
    printf( "decode @ %ld found %d samples\n", packet_seek_timestamp, decode_count );

    bool laminar = true;

        // deck->controls.loop_back_discon.active = true;
        // deck->controls.loop_back_discon.depart_pcm_addr = loop_state->start_addr;
        // deck->controls.loop_back_discon.depart_decode_addr = ???;
        // deck->controls.loop_back_discon.dest_pcm_addr = loop_state->end_addr;

    // // We need to capture any active discon's impact on core_start/end 
    // // Note any discon points falling within the packet
    // if( zdj_decode_address_intersects_packet( 
    //     deck->controls.loop_back_discon.depart_decode_addr,
    //     new_packet ) 
    // ) {
    //     // Address of loop lead-in end
    //     laminar = false;
    //     // Set node core_end

    // } else if( zdj_decode_address_intersects_packet( 
    //     deck->controls.loop_back_discon.depart_decode_addr, 
    //     new_packet ) 
    // ) {
    //     // Address of loop lead-in beginning
    //     // If this packet contains the start of the loop, this is the discon node.
    //     laminar = false;
    //     // Set node core_start
    //     new_packet->type = ZDJ_DECODE_PACKET_TYPE_DISCON;
    // } else 


    if( laminar ) {
        // Laminar state - no discons
        new_packet->av_frame_sample_count = decode_count;
        new_packet->packet_pcm_addr = linked_packet->packet_pcm_addr - new_packet->av_frame_sample_count;
        new_packet->packet_decode_addr = linked_packet->packet_decode_addr - new_packet->av_frame_sample_count;
...
void zdj_decode_add_packet_after_pcm_layer( zdj_pipeline_node_t * node, zdj_decode_layer_t * layer ) {
    ...
    if ( linked_packet->type == ZDJ_DECODE_PACKET_TYPE_END_INERT ) {
        ...
        zdj_decode_make_admin_packet( 
            layer, new_packet, linked_packet,
            ZDJ_DECODE_PACKET_TYPE_END_BUMPER, ZDJ_PACKET_DIRECTION_FWD,
            10000
        );
        ...
        zdj_decode_make_admin_packet( 
            layer, new_packet, linked_packet,
            ZDJ_DECODE_PACKET_TYPE_START_INERT, ZDJ_PACKET_DIRECTION_FWD,
            1000
        );
        ...
        printf("fwd first normal packet @seek: %ld\n", packet_seek_timestamp );
        ...
        zdj_decode_make_admin_packet( 
            layer, new_packet, linked_packet,
            ZDJ_DECODE_PACKET_TYPE_END_INERT, ZDJ_PACKET_DIRECTION_FWD,
            1000
        );
        printf("fwd end inert packet\n" );
        return;
    ...
    printf( "adding post-packet @: %ld\n", packet_seek_timestamp );
    
    new_packet->packet_pcm_addr = linked_packet->packet_pcm_addr + linked_packet->av_frame_sample_count;
...
void zdj_decode_add_packet_to_empty_pcm_layer( zdj_pipeline_node_t * node, zdj_decode_layer_t * layer ) {
    ...
    int64_t packet_seek_timestamp = layer->init_map_pcm;
    av_seek_frame( node_state->fmt_ctx, 0, packet_seek_timestamp, AVSEEK_FLAG_FRAME );
    ...
    int64_t packet_pcm_addr = av_packet->pts / new_packet->av_timebase_factor;
    int64_t packet_pcm_offset = layer->init_map_pcm - packet_pcm_addr;

    printf( "adding initial packet - req'd pcm: %ld pcm: %ld req'd decd: %ld decd: %ld\n",
        packet_seek_timestamp,
        packet_pcm_addr,
