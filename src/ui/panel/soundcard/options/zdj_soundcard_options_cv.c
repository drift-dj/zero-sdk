...
void zdj_soundcard_options_update_cv_layout( zdj_view_t * view ) {
    ...
    if( is_output_port ) { gain_state->data.f_val = zdj_signal_db_for_gain( output_source_node->dsp_dto->gain ); }
    else { gain_state->data.f_val = zdj_signal_db_for_gain( page_node->dsp_dto->gain ); }
