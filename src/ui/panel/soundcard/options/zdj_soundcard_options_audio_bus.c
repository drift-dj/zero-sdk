...
static void _toggle_stereo_dialog_exit( zdj_view_t * view, void * data, bool selection );
...
void zdj_soundcard_options_update_audio_bus_layout( zdj_view_t * view ) {
    ...
    if( is_output_port && output_source_node->dsp_dto ) { 
        gain_state->data.f_val = zdj_signal_db_for_gain( output_source_node->dsp_dto->gain ); 
    } else if ( page_node->dsp_dto ) { 
...
static void _handle_stereo( zdj_view_t * view, zdj_control_event_t * _event ) {
    ...
    if( zdj_soundcard_count_input_nodes_to_node_name( &zdj_soundcard->dto, node->name ) > 0 ||
        zdj_soundcard_count_output_nodes_from_node( &zdj_soundcard->dto, node ) > 0
    ) {
        // If the given node has any input or output links, we need to pop an alert
        ...
        dialog_state->handle_dialog_exit = &_toggle_stereo_dialog_exit;
        dialog_state->selection_data = options_state;
        zdj_push_subview( zdj_panel_view( ), dialog, true );
    } else {
        // This is an unlinked node, just toggle stereo
...
static void _toggle_stereo_dialog_exit( zdj_view_t * view, void * data, bool selection ) {
    // printf( "_toggle_dialog_exit %d\n", selection );
    zdj_soundcard_options_state_t * options_state = (zdj_soundcard_options_state_t*)data;

    // bool stereo = zdj_soundcard_get_node_for_name( zdj_soundcard, options_state->config_context->node->name )->stereo;
    // printf( "stereo: %d\n", stereo );
    zdj_soundcard_set_stereo_for_node( 
        options_state->config_context->node, 
        !zdj_soundcard_get_node_for_name( zdj_soundcard, options_state->config_context->node->name )->stereo
    );
    if( options_state->config_context->main_view_cb ) { 
        options_state->config_context->main_view_cb( options_state->config_context ); 
    }

    // Unlink inputs/outputs
    // zdj_soundcard_unlink_all_nodes_from_node( zdj_soundcard, options_state->config_context->node );
