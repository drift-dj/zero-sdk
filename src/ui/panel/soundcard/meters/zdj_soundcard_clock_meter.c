...
zdj_view_t * zdj_new_clock_meter_view( 
    ...
    // Add clock pulse
    // Get xport deck instance for tracking pulse state
    zdj_deck_t * xport_deck = zdj_soundcard->clock_deck;
    zdj_xport_deck_state_t * deck_state = (zdj_xport_deck_state_t*)xport_deck->state;
    ...
    zdj_soundcard_clock_direction_t dir = zdj_soundcard_dto_get_source_for_node_name( &zdj_soundcard->dto, node->name );
    if( dir == ZDJ_SOUNDCARD_CLOCK_DIRECTION_INPUT ) {
        clock_dir = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_MIXER_CLOCK_METER_IN ], NULL );
    } else {
        clock_dir = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_MIXER_CLOCK_METER_OUT ], NULL );
    }
    clock_dir->frame.x = 1;
    clock_dir->frame.y = 7;
    clock_dir->frame.h = 3;
    state->clock_dir = clock_dir;
    zdj_add_subview( clock_meter_view, clock_dir );
...
void _zdj_clock_meter_draw( zdj_view_t * view, zdj_view_clip_t * clip ) {
    ...
    // Update BPM
    if( state->clock_bpm ){ zdj_remove_subview_of( view, state->clock_bpm ); }
    char str[ 16 ];
    snprintf( str, sizeof( str ), "%1.2f", deck_state->set_bpm );
    zdj_view_t * bpm = zdj_new_label_vert_view( str, ZDJ_FONT_6, ZDJ_JUSTIFY_LEFT, ZDJ_SDL_WHITE );
    bpm->frame.y = 40 - bpm->frame.h;
    ...
    // Update direction arrow if changes
    if( state->clock_dir ){ zdj_remove_subview_of( view, state->clock_dir ); }
    zdj_view_t * clock_dir;
    zdj_soundcard_clock_direction_t dir = zdj_soundcard_dto_get_source_for_node_name( &zdj_soundcard->dto, ZDJ_SOUNDCARD_NODE_NAME_XPORT_0 );
    ...
    clock_dir->frame.h = 3;
    state->clock_dir = clock_dir;
    zdj_add_subview( view, clock_dir );
}
