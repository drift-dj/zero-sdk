...
static void _deploy( zdj_view_t * view, zdj_volume_widget_state_t * widget_state );
static void _retract( zdj_view_t * view, zdj_volume_widget_state_t * widget_state );

static void _draw_container( zdj_view_t * view, zdj_view_clip_t * clip );

static void _init_ui( zdj_view_t * view, zdj_volume_widget_state_t * widget_state );
...
zdj_view_t * zdj_new_volume_widget( void ) {
    ...
    // Add state
    zdj_volume_widget_state_t * state = calloc( 1, sizeof( zdj_volume_widget_state_t ) );
    view->state = state;
    state->ui_init = false;
...
static void _init_ui( zdj_view_t * view, zdj_volume_widget_state_t * widget_state ) {
    ...
    widget_state->meter = zdj_new_audio_stereo_meter_view( 
        lr_node, 
        ZDJ_SOUNDCARD_LABEL_MAIN_BUS, 
        false 
    );
    widget_state->meter->frame.x = 2;
    widget_state->meter->frame.y = 2;
