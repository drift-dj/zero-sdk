...
#define ZDJ_TEXT_INPUT_VIEW_COLUMN_WIDTH 7
#define ZDJ_TEXT_INPUT_VIEW_ROW_HEIGHT 10
...
static void _update_key_layout( zdj_view_t * view );
static void _update_space_key_layout( zdj_view_t * view );
static void _update_shift_key_layout( zdj_view_t * view );
static void _update_backspace_key_layout( zdj_view_t * view );
static void _update_insert_key_layout( zdj_view_t * view );
...
zdj_view_t * zdj_new_text_input_keyboard_view( void ) {
    ...
    zdj_rect_t menu_rect = {0, 0, ZDJ_SCREEN_W, ZDJ_SCREEN_H-22};
    ...
    zdj_view_t * space_key = zdj_new_menu_item( "space" );
    zdj_menu_item_view_state_t * space_state = (zdj_menu_item_view_state_t*)space_key->state;
    space_state->update_layout = _update_space_key_layout;
    space_key->frame->x = 110;
    space_key->frame->y = 9;
    space_key->frame->w = 8;
    space_key->frame->h = 5;
    zdj_menu_view_add_chrome_item( menu, space_key );

    // Add shift key
    _shift_key_index = menu_state->item_count;
    _shift_key = zdj_new_menu_item( "shift" );
    zdj_menu_item_view_state_t * shift_state = (zdj_menu_item_view_state_t*)_shift_key->state;
    shift_state->update_layout = _update_shift_key_layout;
    _shift_key->frame->x = 120;
    _shift_key->frame->y = 7;
    _shift_key->frame->w = 5;
    _shift_key->frame->h = 6;
    zdj_menu_view_add_chrome_item( menu, _shift_key );

    // Add backspace key
    _backspace_key_index = menu_state->item_count;
    zdj_view_t * backspace_key = zdj_new_menu_item( "backspace" );
    zdj_menu_item_view_state_t * backspace_state = (zdj_menu_item_view_state_t*)backspace_key->state;
    backspace_state->update_layout = _update_backspace_key_layout;
    backspace_key->frame->x = 104;
    backspace_key->frame->y = 16;
    backspace_key->frame->w = 9;
    backspace_key->frame->h = 7;
    zdj_menu_view_add_chrome_item( menu, backspace_key );

    // Add insert key
    _insert_key_index = menu_state->item_count;
    zdj_view_t * insert_key = zdj_new_menu_item( "insert" );
    zdj_menu_item_view_state_t * insert_state = (zdj_menu_item_view_state_t*)insert_key->state;
    insert_state->update_layout = _update_insert_key_layout;
    insert_key->frame->x = 116;
    insert_key->frame->y = 16;
    insert_key->frame->w = 9;
    insert_key->frame->h = 7;
    zdj_menu_view_add_chrome_item( menu, insert_key );

    // Add cancel btn
    _cancel_key_index = menu_state->item_count;
    zdj_view_t * cancel_key = zdj_new_menu_item( "Cancel" );
    zdj_menu_item_view_state_t * cancel_state = (zdj_menu_item_view_state_t*)cancel_key->state;
    cancel_state->update_layout = &zdj_menu_item_basic_r_update_layout;
    cancel_key->frame->x = 101;
    cancel_key->frame->y = 24;
    cancel_key->frame->w = 26;
    cancel_key->frame->h = 8;
    zdj_menu_view_add_chrome_item( menu, cancel_key );

    // Add okay btn
    _okay_key_index = menu_state->item_count;
    zdj_view_t * ok_key = zdj_new_menu_item( "Okay" );
    zdj_menu_item_view_state_t * ok_state = (zdj_menu_item_view_state_t*)ok_key->state;
    ok_state->update_layout = &zdj_menu_item_basic_r_update_layout;
    ok_key->frame->x = 108;
    ok_key->frame->y = 32;
    ok_key->frame->w = 19;
    ok_key->frame->h = 8;
    zdj_menu_view_add_chrome_item( menu, ok_key );
        ...
        for( int col=0; col<ZDJ_TEXT_INPUT_KEYBOARD_COLUMN_COUNT; col++ ) {
            ...
            for( int key_ind=0; key_ind<ZDJ_TEXT_INPUT_KEYBOARD_KEY_COUNT; key_ind++ ) {
                if( key->chars[ key_ind ].ascii_char == c ) {
                    ...
                    item_state->has_valid_display = false;
                }
            }
        }
    }
...
void zdj_text_input_keyboard_activate_shift_key( zdj_view_t * keyboard_menu ) {
    zdj_menu_item_view_state_t * shift_state = (zdj_menu_item_view_state_t*)_shift_key->state;
    shift_state->has_valid_display = false;
...
void _update_key_layout( zdj_view_t * view ) {
    ...
    // Setup hilite view
    zdj_view_t * bg = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_HILITE_LEFT ], NULL );
    bg->frame->w = ZDJ_TEXT_INPUT_VIEW_COLUMN_WIDTH;
    zdj_add_subview( state->hilite_view, bg );
    zdj_view_t * bg_r = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_HILITE_RIGHT ], NULL );
...
void _update_space_key_layout( zdj_view_t * view ) {
    zdj_menu_item_view_state_t * state = (zdj_menu_item_view_state_t*)view->state;
    ...
    // Setup normal view
    zdj_view_t * key = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_SPACE_KEY ], NULL );
    zdj_add_subview( state->normal_view, key );
    
    // Setup hilite view
    zdj_view_t * key_hi = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_SPACE_KEY_HI ], NULL );
    zdj_add_subview( state->hilite_view, key_hi );

    state->has_valid_display = true;
}

void _update_shift_key_layout( zdj_view_t * view ) {
    zdj_menu_item_view_state_t * state = (zdj_menu_item_view_state_t*)view->state;
...
void _update_backspace_key_layout( zdj_view_t * view ) {
    zdj_menu_item_view_state_t * state = (zdj_menu_item_view_state_t*)view->state;
    ...
    // Setup normal view
    zdj_view_t * key = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_BACKSPACE_KEY ], NULL );
    zdj_add_subview( state->normal_view, key );
    
    // Setup hilite view
    zdj_view_t * key_hi = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_BACKSPACE_KEY_HI ], NULL );
    zdj_add_subview( state->hilite_view, key_hi );

    state->has_valid_display = true;
}

void _update_insert_key_layout( zdj_view_t * view ) {
    zdj_menu_item_view_state_t * state = (zdj_menu_item_view_state_t*)view->state;
    ...
    // Setup normal view
    zdj_view_t * key = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_INSERT_KEY ], NULL );
    zdj_add_subview( state->normal_view, key );
    
    // Setup hilite view
    zdj_view_t * key_hi = zdj_new_asset_view( &zdj_ui_assets[ ZDJ_UI_ASSET_INSERT_KEY_HI ], NULL );
    zdj_add_subview( state->hilite_view, key_hi );
...
void _add_key_item( zdj_view_t * menu, zdj_keyboard_key_t * key, int row, int col ) {
    char title[ 2 ] = { key->chars[ key->cur_char ].ascii_char, '\0' };
    zdj_view_t * item = zdj_new_menu_item( title );
    zdj_menu_item_view_state_t * state = (zdj_menu_item_view_state_t*)item->state;
    state->update_layout = _update_key_layout;
    state->data->ptr = key;
    item->state = state;
    item->frame->x = col * ZDJ_TEXT_INPUT_VIEW_COLUMN_WIDTH;
