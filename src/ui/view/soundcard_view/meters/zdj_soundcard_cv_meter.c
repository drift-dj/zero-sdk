...
static void _zdj_cv_meter_handle_hmi( zdj_view_t * view, void * _event );
...
zdj_view_t * zdj_new_cv_meter_view( 
    ...
    cv_meter_view->handle_hmi_event = &_zdj_cv_meter_handle_hmi;
...
void _zdj_cv_meter_draw( zdj_view_t * view, zdj_view_clip_t * clip ) {
    ...
    if( config->node->signal_type == ZDJ_SOUNDCARD_SIGNAL_CV_UNIPOLAR ) {
        state->cv_baseline->frame->y = 40;
        state->cv_value->frame->y = 20;
    } else if( config->node->signal_type == ZDJ_SOUNDCARD_SIGNAL_CV_BIPOLAR ) {
        state->cv_baseline->frame->y = 20;
        state->cv_value->frame->y = 16;
    }
    if( state->is_hilite ) {
...
void _zdj_cv_meter_handle_hmi( zdj_view_t * view, void * _event ) {
