...
void zdj_library_get_all_libraries( 
    ...
    if( stmt ) {
        while ( (( sql_res = sqlite3_step( stmt ) ) == SQLITE_ROW) && row < result_limit ) {
            ...
            lib->entity_id = sqlite3_column_int ( stmt, 0 );
...
zdj_library_t * zdj_library_get_library_for_entity_id( int entity_id ) {
    ...
    sprintf( _sql, "select * from %s where entity_id=%d", ZDJ_LIBRARY_TABLE_LIBRARY, entity_id );
    sqlite3_stmt * stmt = zdj_sql_prep_row_stepper( _sql, zdj_library_db );
    if( stmt ) {
...
zdj_health_status_t zdj_library_new( void ) {
    ...
    int new_lib_entity_id = zdj_library_increment_entity_count( );
    snprintf( _sql, sizeof( _sql ), "INSERT INTO Library_Entity VALUES(%d, \"Library %d\", \"Song_Links_0\", \"Playlist_Links_0\", \"Curation_Data_Links_0\", \"Setting_Links_0\");\n",
        new_lib_entity_id,
        lib_count
    );
