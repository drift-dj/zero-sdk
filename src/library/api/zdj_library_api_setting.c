...
zdj_library_setting_t * zdj_library_get_setting( int library_entity_id, zdj_library_setting_type_t setting ) {
    ...
    if( stmt ) {
        while ( (( sql_res = sqlite3_step( stmt ) ) == SQLITE_ROW) ) {
            ...
            res->entity_id = sqlite3_column_int ( stmt, 0 );
...
zdj_health_status_t zdj_library_set_int_setting( int library_entity_id, zdj_library_setting_type_t setting, int val ) {
    if( zdj_library_get_setting( library_entity_id, setting ) ) {
        ...
        snprintf( _sql, sizeof( _sql ), 
            "UPDATE Setting_Entity SET i_val=%d WHERE library_entity_id=%d AND type=%d;\n",
            val,
            library_entity_id,
            setting
        );
        ...
        snprintf( _sql, sizeof( _sql ), 
            "INSERT INTO Setting_Entity VALUES(%d, %d, %d, %d, 0, 0, NULL);\n",
            zdj_library_increment_entity_count( ),
            library_entity_id,
            setting,
            val
        );
...
zdj_health_status_t zdj_library_set_bool_setting( int library_entity_id, zdj_library_setting_type_t setting, bool val ) {
    if( zdj_library_get_setting( library_entity_id, setting ) ) {
        // Update if setting exists
        snprintf( _sql, sizeof( _sql ), 
            "UPDATE Setting_Entity SET b_val=%d WHERE library_entity_id=%d AND type=%d;\n",
            val,
            library_entity_id,
            setting
        );
        ...
        // Insert if setting does't exist
        snprintf( _sql, sizeof( _sql ), 
            "INSERT INTO Setting_Entity VALUES(%d, %d, %d, 0, %d, 0, \"\");\n",
            zdj_library_increment_entity_count( ),
            library_entity_id,
            setting,
            val
...
zdj_health_status_t zdj_library_set_float_setting( int library_entity_id, zdj_library_setting_type_t setting, float val ) {
    if( zdj_library_get_setting( library_entity_id, setting ) ) {
        // Update if setting exists
        snprintf( _sql, sizeof( _sql ), 
            "UPDATE Setting_Entity SET f_val=%f WHERE library_entity_id=%d AND type=%d;\n",
            val,
            library_entity_id,
            setting
        );
        ...
        // Insert if setting does't exist
        snprintf( _sql, sizeof( _sql ), 
            "INSERT INTO Setting_Entity VALUES(%d, %d, %d, 0, 0, %f, \"\");\n",
            zdj_library_increment_entity_count( ),
            library_entity_id,
            setting,
            val
...
zdj_health_status_t zdj_library_set_char_setting( int library_entity_id, zdj_library_setting_type_t setting, char * val ) {
    if( zdj_library_get_setting( library_entity_id, setting ) ) {
        // Update if setting exists
        snprintf( _sql, sizeof( _sql ), 
            "UPDATE Setting_Entity SET c_val=%s WHERE library_entity_id=%d AND type=%d;\n",
            val,
            library_entity_id,
            setting
        );
        ...
        // Insert if setting does't exist
        snprintf( _sql, sizeof( _sql ), 
            "INSERT INTO Setting_Entity VALUES(%d, %d, %d, 0, 0, 0, \"%s\");\n",
            zdj_library_increment_entity_count( ),
            library_entity_id,
            setting,
            val
