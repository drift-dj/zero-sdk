...
zdj_error_type_t zdj_library_query_all_genres( 
    ...
    printf( "zdj_library_query_all_genres\n" );
    ...
    if( a_stmt ) {
        while ( ( res = sqlite3_step( a_stmt ) ) == SQLITE_ROW ) {
            ...
            if( row < count ) {
                genres[ row ] = strdup( (char*)sqlite3_column_text ( a_stmt, 0 ) );
            }
            row++;
        }
...
int zdj_library_query_count_songs_in_genre( 
    ...
    printf( "zdj_library_query_count_songs_in_genre\n" );
...
zdj_error_type_t zdj_library_query_songs_in_genre( 
    ...
    printf( "zdj_library_query_songs_in_genre\n" );
    // Populate list of artists
    snprintf( _sql, sizeof( _sql ), 
        "SELECT * from Song_Entity\n"
        "INNER JOIN Catalog_Data_Entity ON Song_Entity.catalog_data_entity_id = Catalog_Data_Entity.entity_id\n"
