...
zdj_error_type_t zdj_library_query_all_years( 
    ...
    printf( "zdj_library_query_all_years\n" );
    ...
    if( a_stmt ) {
        while ( ( res = sqlite3_step( a_stmt ) ) == SQLITE_ROW ) {
            ...
            if( row < count ) {
                years[ row ] = sqlite3_column_int ( a_stmt, 0 );
            }
            row++;
        }
...
int zdj_library_query_count_songs_in_year( 
    ...
    printf( "zdj_library_query_count_songs_in_year\n" );
    ...
    snprintf( _sql, sizeof( _sql ),
        ...
        "(SELECT * from Song_Entity\n"
        "INNER JOIN Catalog_Data_Entity ON Song_Entity.catalog_data_entity_id = Catalog_Data_Entity.entity_id\n"
        "WHERE Song_Entity.entity_id IN (select entity_id from Song_Links_%s) AND year like %d)\n",
        library_entity_id,
        year
    );
...
zdj_error_type_t zdj_library_query_songs_in_year( 
    ...
    printf( "zdj_library_query_songs_in_year\n" );
    // Populate list of artists
    snprintf( _sql, sizeof( _sql ), 
        "SELECT * from Song_Entity\n"
        "INNER JOIN Catalog_Data_Entity ON Song_Entity.catalog_data_entity_id = Catalog_Data_Entity.entity_id\n"
